...
export let shaderMetaball = glsl`
    ...
    void main() {
        ...
        // parameters
        float interval = 2.;
        float size = 0.1*fadeIn*(1.-fadeOut);
        float radius = size;
        const float count = 8.;
        ...
        vec3 background = mix(vec3(1), vec3(ss(1.5, -.5, length(pos))), fadeIn);
        background = mix(background, 0.5 + 0.5 * cos(vec3(0,.3,.6)*6.28+x*6.28+1.5), fadeOut);
        
        // shadow
        background *= mix(clamp(shape*5.+.9, 0., 1.), 1., fadeOut);
